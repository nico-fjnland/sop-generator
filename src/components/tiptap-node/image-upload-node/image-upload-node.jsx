...
const DropZoneContent = ({
  ...
  <>
    ...
    <div className="tiptap-image-upload-content">
      <span className="tiptap-image-upload-text">
        <em>Klicken zum Hochladen</em> oder Bild hierher ziehen
      </span>
      <span className="tiptap-image-upload-subtext">
        Max. {limit} {limit === 1 ? "Datei" : "Dateien"}, {maxSize / 1024 / 1024}MB
      </span>
    </div>
  </>
)
  ...
  const handleUpload = async (files) => {
    ...
    if (urls.length > 0) {
      ...
      if (isValidPosition(pos)) {
        ...
        // Move cursor after the inserted image to deselect it
        setTimeout(() => {
          const { state } = props.editor
          const resolvedPos = state.doc.resolve(Math.min(pos + 2, state.doc.content.size))
          props.editor.commands.setTextSelection(resolvedPos.pos)
        }, 10)
      }
    }
  }
        ...
        <div className="tiptap-image-upload-previews">
          {fileItems.length > 1 && (
            <div className="tiptap-image-upload-header">
              <span>{fileItems.length} Dateien werden hochgeladen</span>
              <Button
                type="button"
                data-style="ghost"
                onClick={(e) => {
